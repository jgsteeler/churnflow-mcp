...
export class FormattingUtils {
   ...
   */
  static formatActivity(description: string, timestamp: Date = new Date()): string {
   ...
   */
  static formatReference(title: string, description: string, date: Date = new Date()): string {
   ...
   */
  static formatSomedayItem(description: string, tag: string, captureDate: Date = new Date()): string {
   ...
   */
  static formatReviewItem(description: string, confidence: number = 0, date: Date = new Date()): string {
   ...
   */
  static extractTags(text: string): { hashtags: string[], contextTags: string[] } {
  ...
  static validateEntryFormat(entry: string, expectedType: ItemType): {
    ...
    switch (expectedType) {
      ...
      case 'activity':
        ...
        if (!timestampMatch || !this.validateTimestampFormat(timestampMatch[1])) {
